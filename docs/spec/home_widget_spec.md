# ホームウィジェット・ロック画面ウィジェット 設計書

## 1. 概要

### 1.1 目的
ユーザーがアプリを開かずに、ホーム画面やロック画面から素早く育児記録を確認・登録できるようにする。

### 1.2 対象プラットフォーム
- **iOS**: Home Screen Widget + Lock Screen Widget (WidgetKit)
- **Android**: Home Screen Widget (App Widget)

### 1.3 ウィジェット種類

| ウィジェット | サイズ | プラットフォーム |
|-------------|--------|-----------------|
| 横長ウィジェット | Medium (iOS) / 4x2 (Android) | iOS, Android |
| 正方形ウィジェット | Small (iOS) / 2x2 (Android) | iOS, Android |
| ロック画面ウィジェット | Circular/Rectangular | iOS のみ |

---

## 2. ウィジェット仕様

### 2.1 横長ウィジェット（Medium / 4x2）

```
┌─────────────────────────────────────────────────────────────┐
│  [👶] たろう  1歳2ヶ月13日                   [切替ボタン]   │
├─────────────────────────────────────────────────────────────┤
│   🤱 授乳(右)    │    🍼 ミルク     │     💧 尿           │
│     12:00       │      14:30       │      13:45          │
│    2時間前      │      30分前      │     1時間前         │
├─────────────────────────────────────────────────────────────┤
│  [🤱]  [🍼]  [💧]  [💩]  [🌡️]                               │
└─────────────────────────────────────────────────────────────┘
```

#### 表示内容
1. **ヘッダー部**
   - 選択中の子供の名前
   - 生後経過日数（例: `1歳2ヶ月13日`、`0ヶ月15日目`）
   - 子供切替ボタン（複数の子供がいる場合のみ表示）

2. **記録カード部（3列レイアウト）**
   - **3種類**の`RecordType`それぞれの直近1件を横並びで表示
   - 表示項目: アイコン、種別名、記録時刻、経過時間（例: `30分前`）
   - 表示する3種類はユーザーが設定で選択可能
   - 該当種別の記録がない場合: 「--」と表示
   - デフォルト: `breastRight`（または`breastLeft`）, `formula`, `pee`

3. **クイックアクション部**
   - 5つの`RecordType`アイコンボタン
   - タップでアプリ起動 → 該当種別の記録ダイアログを表示
   - 表示するアイコンはユーザーがカスタマイズ可能

#### インタラクション
| アクション | 動作 |
|-----------|------|
| ウィジェット全体タップ | アプリ起動（ベビーの記録画面） |
| 子供切替ボタン | 次の子供に切り替え（ウィジェット内で完結） |
| クイックアクションアイコン | アプリ起動 + 記録ダイアログ表示（現在時刻プリセット） |

---

### 2.2 正方形ウィジェット（Small / 2x2）

```
┌─────────────────────┐
│ たろう              │
│ 1歳2ヶ月13日  [⟳]   │
├─────────────────────┤
│ 🍼 ミルク           │
│ 14:30  30分前       │
└─────────────────────┘
```

#### 表示内容
1. **ヘッダー部**
   - 選択中の子供の名前
   - 生後経過日数
   - 子供切替ボタン（複数の子供がいる場合）

2. **記録表示部**
   - 直近1件の`Record`を表示
   - 表示項目: アイコン、種別名、記録時刻、経過時間
   - 記録がない場合: 「記録がありません」

#### フィルタリング機能
- 表示する`RecordType`を**1種類**指定可能
- 指定なし（デフォルト）: 種別に関係なく最新の記録を表示
- 指定あり: 該当種別の最新記録のみ表示
- 例: 「ミルク」を指定 → ミルクの最新記録のみ表示

#### インタラクション
| アクション | 動作 |
|-----------|------|
| ウィジェット全体タップ | アプリ起動（ベビーの記録画面） |
| 子供切替ボタン | 次の子供に切り替え |

---

### 2.3 ロック画面ウィジェット（iOS専用）

#### Circular（円形）
```
┌─────┐
│ 🍼  │
│30分前│
└─────┘
```

#### Rectangular（長方形）
```
┌─────────────────┐
│ 🍼 ミルク 14:30 │
│    30分前       │
└─────────────────┘
```

#### 表示内容
- 選択中の子供の最新`Record`
- 表示項目: アイコン、種別名（Rectangularのみ）、記録時刻、経過時間

#### インタラクション
- タップでアプリ起動

---

## 3. データ連携設計

### 3.1 課題
- ウィジェットはアプリプロセス外で動作するため、Firestoreに直接アクセスできない
- ウィジェットの更新頻度に制限がある（iOS: Timeline、Android: updatePeriodMillis）
- アプリとウィジェット間でデータを共有する仕組みが必要

### 3.2 アーキテクチャ

```
┌─────────────────────────────────────────────────────────────┐
│                      Flutter App                            │
│  ┌─────────────┐    ┌─────────────┐    ┌────────────────┐  │
│  │  Firestore  │───▶│  UseCase    │───▶│WidgetDataSync  │  │
│  │  (Remote)   │    │             │    │   Service      │  │
│  └─────────────┘    └─────────────┘    └───────┬────────┘  │
└───────────────────────────────────────────────┬─────────────┘
                                                │
                    ┌───────────────────────────▼──────────────┐
                    │         Shared Storage                   │
                    │  ┌─────────────────────────────────────┐ │
                    │  │ iOS: App Group (UserDefaults)       │ │
                    │  │ Android: SharedPreferences          │ │
                    │  └─────────────────────────────────────┘ │
                    └───────────────────────────┬──────────────┘
                                                │
┌───────────────────────────────────────────────▼──────────────┐
│                    Native Widget                             │
│  ┌─────────────────┐              ┌─────────────────┐       │
│  │ iOS: WidgetKit  │              │ Android: AppWidget│      │
│  └─────────────────┘              └─────────────────┘       │
└──────────────────────────────────────────────────────────────┘
```

### 3.3 共有データ構造

#### WidgetData（JSON形式で保存）

```json
{
  "lastUpdated": "2025-12-02T14:30:00.000Z",
  "selectedChildId": "child_abc123",
  "children": [
    {
      "id": "child_abc123",
      "name": "たろう",
      "birthday": "2024-09-20"
    },
    {
      "id": "child_def456",
      "name": "はなこ",
      "birthday": "2023-05-15"
    }
  ],
  "recentRecords": {
    "child_abc123": [
      {
        "id": "2025-12-02-14-formula-xxx",
        "type": "formula",
        "at": "2025-12-02T14:30:00.000Z",
        "amount": 120,
        "note": null
      },
      {
        "id": "2025-12-02-13-pee-yyy",
        "type": "pee",
        "at": "2025-12-02T13:45:00.000Z",
        "amount": null,
        "excretionVolume": "medium"
      }
    ],
    "child_def456": [
      // ...
    ]
  }
}
```

#### WidgetSettings（ユーザー設定）

```json
{
  "mediumWidget": {
    "displayRecordTypes": ["breastRight", "formula", "pee"],
    "quickActionTypes": ["breastRight", "formula", "pee", "poop", "temperature"]
  },
  "smallWidget": {
    "filterRecordType": null
  }
}
```

| 設定項目 | ウィジェット | 説明 |
|---------|-------------|------|
| `displayRecordTypes` | Medium | 3列で表示する3種類のRecordType |
| `quickActionTypes` | Medium | 下部に表示する5つのクイックアクション |
| `filterRecordType` | Small | 表示するRecordType（nullで全種類）|

### 3.4 データ同期タイミング

| イベント | 同期内容 |
|---------|---------|
| アプリ起動時 | 全データを同期 |
| Record追加/更新/削除時 | 該当childのrecentRecordsを更新 |
| 子供追加/更新/削除時 | childrenリストを更新 |
| 選択中の子供変更時 | selectedChildIdを更新 |
| ウィジェット設定変更時 | WidgetSettingsを更新 |

### 3.5 ローカルストレージ実装

#### iOS: App Group（Flavor対応）

環境ごとに異なるApp Group IDを使用：

| Flavor | App Group ID |
|--------|--------------|
| stg | `group.com.aphlo.babymomdiary.stg` |
| prod | `group.com.aphlo.babymomdiary` |

```swift
// STG環境
let userDefaults = UserDefaults(suiteName: "group.com.aphlo.babymomdiary.stg")

// PROD環境
let userDefaults = UserDefaults(suiteName: "group.com.aphlo.babymomdiary")

userDefaults?.set(jsonString, forKey: "widget_data")
```

#### Android: SharedPreferences
```kotlin
// Mode: MODE_PRIVATE with ContentProvider for widget access
val prefs = context.getSharedPreferences("widget_data", Context.MODE_PRIVATE)
prefs.edit().putString("widget_data", jsonString).apply()
```

#### Flutter側（home_widget パッケージ使用）
```dart
import 'package:home_widget/home_widget.dart';

// アプリ起動時にApp Group IDを設定（main_stg.dart / main_prod.dart）
await WidgetDataRepositoryImpl.initialize(isProduction: false); // STG
await WidgetDataRepositoryImpl.initialize(isProduction: true);  // PROD

// データ保存
await HomeWidget.saveWidgetData<String>('widget_data', jsonEncode(widgetData));

// ウィジェット更新通知
await HomeWidget.updateWidget(
  iOSName: 'MiluWidget',
  androidName: 'MiluWidgetProvider',
);
```

### 3.6 Flavor対応の詳細

#### Entitlements ファイル構成

```
ios/
├── Runner/
│   ├── Runner-stg.entitlements    # STG用 (group.com.aphlo.babymomdiary.stg)
│   └── Runner-prod.entitlements   # PROD用 (group.com.aphlo.babymomdiary)
└── MiluWidget/
    ├── MiluWidget-stg.entitlements  # STG用
    └── MiluWidget-prod.entitlements # PROD用
```

#### Xcode Build Configuration 設定

**Runner Target:**
| Configuration | CODE_SIGN_ENTITLEMENTS |
|---------------|------------------------|
| Debug-stg, Release-stg, Profile-stg | `Runner/Runner-stg.entitlements` |
| Debug-prod, Release-prod, Profile-prod | `Runner/Runner-prod.entitlements` |

**MiluWidget Target:**
| Configuration | CODE_SIGN_ENTITLEMENTS | PRODUCT_BUNDLE_IDENTIFIER |
|---------------|------------------------|---------------------------|
| Debug-stg, Release-stg, Profile-stg | `MiluWidget/MiluWidget-stg.entitlements` | `com.aphlo.babymomdiary.stg.MiluWidget` |
| Debug-prod, Release-prod, Profile-prod | `MiluWidget/MiluWidget-prod.entitlements` | `com.aphlo.babymomdiary.MiluWidget` |

---

## 4. ディープリンク設計

### 4.1 URLスキーム

```
milu://record/add?type={RecordType}&time={ISO8601}
```

#### パラメータ
| パラメータ | 必須 | 説明 | 例 |
|-----------|------|------|-----|
| type | Yes | RecordType名 | `formula`, `breastRight` |
| time | No | プリセット時刻（デフォルト: 現在時刻） | `2025-12-02T14:30:00` |

#### 例
```
milu://record/add?type=formula&time=2025-12-02T14:30:00
milu://record/add?type=pee
```

### 4.2 アプリ内ルーティング

```dart
// go_router設定
GoRoute(
  path: '/record/add',
  builder: (context, state) {
    final type = RecordType.values.byName(state.queryParams['type']!);
    final time = state.queryParams['time'] != null
        ? DateTime.parse(state.queryParams['time']!)
        : DateTime.now();
    return AddRecordPage(presetType: type, presetTime: time);
  },
)
```

---

## 5. ウィジェット設定画面（アプリ内）

### 5.1 設定項目

#### 横長ウィジェット設定（Medium）

| 設定項目 | 説明 | デフォルト |
|---------|------|-----------|
| 表示する記録種別 | 3列で表示する3種類のRecordTypeを選択 | 授乳(右), ミルク, 尿 |
| クイックアクション | 下部に表示する5つのRecordTypeを選択 | 授乳(右), ミルク, 尿, 便, 体温 |

- ドラッグ&ドロップで並び替え可能
- 記録種別とクイックアクションは独立して設定可能

#### 正方形ウィジェット設定（Small）

| 設定項目 | 説明 | デフォルト |
|---------|------|-----------|
| 表示する記録種別 | 表示する1種類のRecordTypeを選択（または「すべて」） | すべて |

- 「すべて」選択時は種別に関係なく最新の記録を表示

---

## 5.5 記録表示ルール

### 経過時間の表示形式

記録の経過時間は以下の形式で表示する:

| 経過時間 | 表示形式 | 例 |
|---------|---------|-----|
| 1分未満 | 「たった今」 | たった今 |
| 1分〜59分 | 「X分前」 | 30分前 |
| 1時間〜23時間59分 | 「X時間Y分前」（分がある場合） | 1時間30分前、2時間前 |
| 24時間以上 | 表示しない（記録非表示） | - |

### 24時間フィルター

**重要**: ウィジェットには24時間以内の記録のみ表示する。

- 24時間以上前の記録は、種別に関係なくウィジェット上に表示されない
- 未来の記録（予約記録など）も表示されない
- 該当する記録がない場合、時刻は「--」と表示される
- このフィルターはスモールウィジェット・ミディアムウィジェット両方に適用される

#### 実装箇所

フィルタリングは**2段階**で行われる：

1. **Flutter側（データ保存時）**: `WidgetDataSyncService._filterValidRecords()`
   - ローカルストレージに保存する前に24時間以上前・未来の記録を除外
   - ストレージ効率とパフォーマンス向上のため

2. **ネイティブ側（表示時）**: 各WidgetProviderの`findRecordByTypeOrCategory()` / `findLatestPastRecord()`
   - データ保存後に時間が経過して24時間を超える可能性があるため
   - 防御的プログラミングとして二重チェック

#### 理由
- ウィジェットは「最新の育児状況」を素早く確認するためのもの
- 24時間以上前の情報は実用性が低く、むしろ誤解を招く可能性がある
- スペースを有効活用し、本当に必要な情報のみを表示する

---

### 5.2 UI配置
- メニュー → 設定 → ウィジェット設定

### 5.3 設定画面ワイヤーフレーム

```
┌─────────────────────────────────────┐
│ ← ウィジェット設定                  │
├─────────────────────────────────────┤
│                                     │
│ ■ 横長ウィジェット                  │
│ ─────────────────────────────────── │
│                                     │
│ 表示する記録種別（3つ選択）          │
│ ┌─────┐ ┌─────┐ ┌─────┐            │
│ │ 🤱  │ │ 🍼  │ │ 💧  │            │
│ │授乳 │ │ミルク│ │ 尿  │            │
│ └─────┘ └─────┘ └─────┘            │
│                        [変更]       │
│                                     │
│ クイックアクション（5つ選択）        │
│ ┌───┐┌───┐┌───┐┌───┐┌───┐         │
│ │🤱 ││🍼 ││💧 ││💩 ││🌡️ │         │
│ └───┘└───┘└───┘└───┘└───┘         │
│                        [変更]       │
│                                     │
│ ■ 正方形ウィジェット                │
│ ─────────────────────────────────── │
│                                     │
│ 表示する記録種別                     │
│ ○ すべて                           │
│ ○ 授乳                            │
│ ○ ミルク                           │
│ ○ 搾母乳                           │
│ ○ 尿                               │
│ ○ 便                               │
│ ○ 体温                             │
│ ○ その他                           │
│                                     │
└─────────────────────────────────────┘
```

---

## 6. 技術スタック

### 6.1 Flutter側

| パッケージ | 用途 |
|-----------|------|
| [home_widget](https://pub.dev/packages/home_widget) | ウィジェットとのデータ共有、更新通知 |
| [shared_preferences](https://pub.dev/packages/shared_preferences) | ウィジェット設定の永続化 |

### 6.2 iOS側

| 技術 | 用途 |
|------|------|
| WidgetKit | ホーム画面/ロック画面ウィジェット |
| SwiftUI | ウィジェットUI |
| App Groups | アプリ-ウィジェット間データ共有 |

### 6.3 Android側

| 技術 | 用途 |
|------|------|
| App Widget | ホーム画面ウィジェット |
| Jetpack Glance | ウィジェットUI（Compose風） |
| SharedPreferences | アプリ-ウィジェット間データ共有 |

---

## 7. アプリ側実装（Flutter）

### 7.1 新規作成ファイル

```
flutter/lib/src/features/widget/
├── domain/
│   ├── entities/
│   │   ├── widget_data.dart           # ウィジェット共有データ
│   │   └── widget_settings.dart       # ウィジェット設定
│   └── repositories/
│       └── widget_data_repository.dart # リポジトリインターフェース
├── application/
│   ├── usecases/
│   │   ├── sync_widget_data.dart      # データ同期UseCase
│   │   └── update_widget_settings.dart # 設定更新UseCase
│   └── providers/
│       └── widget_providers.dart       # Riverpod Providers
├── infrastructure/
│   ├── repositories/
│   │   └── widget_data_repository_impl.dart
│   └── services/
│       └── widget_data_sync_service.dart # 同期サービス
└── presentation/
    ├── pages/
    │   └── widget_settings_page.dart   # 設定画面
    └── viewmodels/
        └── widget_settings_viewmodel.dart
```

### 7.2 データ同期サービス

```dart
/// ウィジェットデータ同期サービス
class WidgetDataSyncService {
  final ChildRecordRepository _recordRepository;
  final ChildrenRepository _childrenRepository;

  /// Record変更時に呼び出し
  Future<void> onRecordChanged(String childId) async {
    final records = await _fetchRecentRecords(childId, limit: 10);
    await _updateWidgetData(childId, records);
    await HomeWidget.updateWidget(
      iOSName: 'MiluWidget',
      androidName: 'MiluWidgetProvider',
    );
  }

  /// 子供情報変更時に呼び出し
  Future<void> onChildrenChanged(List<ChildSummary> children) async {
    await _updateChildrenInWidgetData(children);
    await HomeWidget.updateWidget(...);
  }

  /// 選択子供変更時に呼び出し
  Future<void> onSelectedChildChanged(String childId) async {
    await _updateSelectedChildInWidgetData(childId);
    await HomeWidget.updateWidget(...);
  }
}
```

### 7.3 既存コードへの統合

#### Record追加時
```dart
// AddRecordUseCase
class AddRecord {
  final ChildRecordRepository _repository;
  final WidgetDataSyncService _widgetSync;

  Future<void> call({required String childId, required Record record}) async {
    await _repository.addRecord(childId, record);

    // ウィジェットデータを同期
    await _widgetSync.onRecordChanged(childId);
  }
}
```

---

## 8. ネイティブ側実装概要

### 8.1 iOS (WidgetKit + SwiftUI)

```swift
// MiluWidget.swift
struct MiluWidget: Widget {
    let kind: String = "MiluWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            MiluWidgetEntryView(entry: entry)
        }
        .configurationDisplayName("milu")
        .description("育児記録をすばやく確認")
        .supportedFamilies([.systemSmall, .systemMedium, .accessoryCircular, .accessoryRectangular])
    }
}

struct Provider: TimelineProvider {
    func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        // App Groupからデータ読み込み
        let data = loadWidgetData()
        let entry = MiluEntry(date: Date(), data: data)

        // 15分後に更新
        let nextUpdate = Calendar.current.date(byAdding: .minute, value: 15, to: Date())!
        let timeline = Timeline(entries: [entry], policy: .after(nextUpdate))
        completion(timeline)
    }
}
```

### 8.2 Android (Jetpack Glance)

```kotlin
// MiluWidgetProvider.kt
class MiluWidgetProvider : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        val data = loadWidgetData(context)

        provideContent {
            MiluWidgetContent(data)
        }
    }
}

@Composable
fun MiluWidgetContent(data: WidgetData) {
    Column(
        modifier = GlanceModifier
            .fillMaxSize()
            .background(Color.White)
            .clickable(actionStartActivity<MainActivity>(
                actionParametersOf(
                    ActionParameters.Key<String>("deeplink") to "milu://record/add?type=formula"
                )
            ))
    ) {
        // UI実装
    }
}
```

---

## 9. 更新頻度とバッテリー考慮

### 9.1 iOS
- **Timeline更新**: 15分間隔
- **アプリからの強制更新**: `WidgetCenter.shared.reloadAllTimelines()`
- **Background App Refresh**: 有効時により頻繁に更新可能

### 9.2 Android
- **updatePeriodMillis**: 30分（最小値）
- **アプリからの強制更新**: `AppWidgetManager.updateAppWidget()`
- **WorkManager**: より細かい制御が必要な場合

### 9.3 最適化
- 記録変更時のみ強制更新を実行
- Timeline/updatePeriodMillisは「経過時間」の表示更新用

---

## 10. 実装優先度

| 優先度 | 項目 |
|--------|------|
| P0 | Flutter側のデータ同期基盤 |
| P0 | iOS横長ウィジェット |
| P0 | Android横長ウィジェット |
| P1 | iOS正方形ウィジェット |
| P1 | Android正方形ウィジェット |
| P1 | iOSロック画面ウィジェット |
| P2 | ウィジェット設定画面 |
| P2 | クイックアクションカスタマイズ |
