...
**備考**: カスタム食材の追加・編集・削除はメニュー画面から行う（記録画面からは追加しない）。
...
メニュー画面から「離乳食の食材管理」をタップして開く。カスタム食材の追加・削除ができる。
...
| 機能 | 説明 |
|------|------|
| カテゴリ展開 | タップでアコーディオン展開/閉じる |
| 食材追加 | 「＋ 食材を追加」をタップしてダイアログで追加 |
| 食材削除 | カスタム食材・デフォルト食材ともに削除可能 |
| 食材復元 | 削除したデフォルト食材は「削除した食材」セクションに表示され、復元可能 |
...
### 5.3 非表示食材（削除したデフォルト食材）

```
households/{householdId}/settings/hidden_ingredients
└── ingredient_ids: [String]  // 非表示にした食材名のリスト
```
...
1. メニュー画面から「離乳食の食材管理」を開く
2. カテゴリを選択
3. 「追加」ボタンをタップ
4. ダイアログで食材名を入力
5. 「追加」をタップ
6. カスタム食材がリストに追加され、離乳食記録画面で選択可能になる

**備考**: 離乳食記録画面からはカスタム食材の追加はできない。ヘルプリンクでメニュー画面からの追加を案内する。
