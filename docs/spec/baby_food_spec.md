...
メニュー画面から「離乳食の食材管理」をタップして表示する画面。
食材の追加・非表示（表示/非表示切替）ができる。
...
| 機能 | 説明 |
|------|------|
| 食材追加 | 「＋ 食材を追加」でカスタム食材を追加 |
| 食材非表示 | ✕ボタンで食材を非表示にする（削除ではない） |
| 食材復元 | 「復元」ボタンで非表示食材を表示に戻す |

#### 非表示の影響範囲

非表示にした食材は以下の画面に表示されなくなる：
- 食材管理画面の「表示する食材」セクション
- 離乳食タブの食材一覧
- 離乳食記録画面の食材選択

**注意**: 非表示は「削除」ではなく、過去の記録には影響しない。復元すれば再び表示される。
...
```
households/{householdId}/hidden_ingredients/{documentId}
├── ingredient_ids: [String]  // 非表示にした食材のID（プリセット食材は食材名、カスタム食材はID）
└── updated_at: Timestamp
```

**備考**:
- プリセット食材は食材名（例: "米", "うどん"）をIDとして使用
- カスタム食材はFirestoreのドキュメントID（UUID）を使用
- 同一の `ingredient_ids` 配列でプリセット・カスタム両方を管理
...
| 2025-12-30 | 1.6 | メニュー画面に「離乳食の食材管理」を復活。タブベースのUIに変更。デフォルト食材・カスタム食材両方の非表示/復元機能を追加。非表示食材は離乳食タブにも反映される。 |
