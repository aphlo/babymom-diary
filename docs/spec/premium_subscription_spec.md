...
┌─────────────────────┬───────────────┐
  ...
  つまり、トライアル加入後3分待てば自動的に有料期間に移行し、その後5分ごと（月額の場合）に更新されて6回で停止します。停止後にサブスクが失効するので、広告
  復帰などを確認できます。
  ...
  - 月額プランで購入完了 → ページが閉じる → メニューに「プレミアム加入中」表示
  - 年額プランで購入完了 → 同上
  - 購入ダイアログでキャンセル → SnackBar なし、ページはそのまま
  ...
  - トライアル加入直後 → isPremium: true、広告非表示
  - 3分後 → 自動的に有料期間に移行（特に変化なし、プレミアム継続）

  サブスク失効

  - 6回更新後（または端末の 設定 → App Store → サンドボックスアカウント → サブスクリプション管理 でキャンセル）
  - 失効後 → 広告が復帰する
  - 失効後 → メニューに「プレミアムプラン」（未加入表示）に戻る

  リストア

  - サブスク加入済みの状態でアプリを削除 → 再インストール
  - ペイウォール → 「購入を復元」→ プレミアム状態が復帰

  エッジケース

  - 機内モードで購入 → 適切なエラーメッセージ
  - 2回目のトライアル申込 → Apple が自動的にトライアルなしで課金（アプリ側の対応不要）

  Sandbox でのサブスク管理場所

  端末の 設定 → App Store → サンドボックスアカウント → サブスクリプション管理 からキャンセルや確認ができます。