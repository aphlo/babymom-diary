...
| Android | `SimpleDialog`（タイトルは `titleMedium` スタイル） |

#### ユーザー選択時の動作

| 選択 | 動作 |
|------|------|
| 満足 | In-App Reviewダイアログを表示（OS純正） |
| 不満 | 第2段階（フィードバック確認ダイアログ）を表示 |
...
### 2.3 不満選択時：フィードバック確認ダイアログ

「不満」を選択した場合、フィードバック確認ダイアログを表示する。
...
| 選択 | 動作 |
|------|------|
| 意見を送る | 外部ブラウザでKoeLoopフィードバックページを開く |
| 今はしない | ダイアログを閉じる |
| ダイアログ外タップ/戻る | ダイアログを閉じる |

**フィードバックURL:** KoeLoop埋め込みページ（メニューのお問い合わせと同一）
...
| 条件 | 説明 |
|------|------|
| レビュー済み | ユーザーが過去に「満足」を選択した |
| 不満選択から1ヶ月以内 | ユーザーが過去に「不満」を選択してから30日以内 |
...
| `review_has_reviewed` | bool | レビュー済みフラグ（満足選択時にON） | false |
| `review_dissatisfied_date` | String? | 不満を選択した日付（ISO8601形式、日付のみ） | null |
...
2. ユーザーが「不満」を選択
3. 不満選択日を記録 → **以降1ヶ月間（30日間）ダイアログは表示されない**
4. フィードバック確認ダイアログを表示
5. ユーザーが「意見を送る」を選択
6. 外部ブラウザでKoeLoopフィードバックページを開く
...
| 項目 | URL |
|------|-----|
| KoeLoopフィードバック | `https://koeloop.dev/embed/dddb40ea-a331-4cb9-84bb-b81187047a20?...` |

※ メニューのお問い合わせページと同一のKoeLoop埋め込みURLを使用
...
| 2026-01-13 | 1.2 | 不満選択時に2段階目のフィードバック確認ダイアログを追加。遷移先をKoeLoopに変更。プラットフォーム別UI（iOS: CupertinoAlertDialog、Android: SimpleDialog）を明記。 |
